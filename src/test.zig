...
test "parse multiple directories with short flag" {
    ...
    defer result.deinit(std.testing.allocator, .{ .free_arrays = true });

    const directories = result.getArg("directory").?.parsed.array.string;
    ...
    const parent = result.getFlag("parents").?.parsed.boolean;
